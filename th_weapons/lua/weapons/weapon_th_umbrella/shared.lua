...
SWEP.MissSound 		= 'weapon_th_umbrella.single'

-- The sound to play when hitting a living entity. .i.e: player; npc.
SWEP.HitSound 		= 'weapon_th_umbrella.melee_hit'

-- The sound to play when hitting a non living entity. .i.e: world
SWEP.HitWorldSound 	= 'weapon_th_umbrella.melee_hitworld'
...
local umbrella_damage = GetConVar( 'sk_th_plr_dmg_umbrella' ) or CreateConVar( 'sk_th_plr_dmg_umbrella', '10' )	
...
function SWEP:Swing()
	...
	if tr.Fraction >= 1.0 then
		...
		local swing = ( self:GetSwingCount() % #self.MissSequences )
		local seq = self:LookupSequence( self.MissSequences[ swing + 1 ] )
		...
		dmgInfo:SetDamage( umbrella_damage:GetFloat() )
